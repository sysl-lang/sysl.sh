...
class DocsTests extends AnyFreeSpec with DocsSupport with ParallelTestExecution {
  ...
  private val expected: Map[String, (Int, Int, Int)] = Map(
    ...
    // field changed twice). One more of each again for the identities: every integer type is a
    // member of `Zero` and `One` now, so a generic sum runs over an `int` and over a `real` from one
    // body, and a constrained subtype is refused by name because a range need not hold the value.
    "docs/content/reference/expressions.md"           -> (34, 18, 0),
