...
| [`sysl.math.matrix`](/library/matrix/) | `Vector[T]` and `Matrix[T]` over any `Scalar` — the four products of a vector space, Gaussian elimination for a field, and Bareiss's fraction-free one for a ring | — |
